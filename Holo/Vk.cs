        ...
        public static void StartBot()
        {
            ...
            while (botActive)
            {
                ...
                else
                {
                    ...
                    {
                        Thread thread = new Thread(() => HandleEvents(event_));
                        thread.Start();
                    }
                    ts = events.Ts;
                }
        ...
        public static void SendMessage(string text, long id, string filepath = "")
        {
            try
            {
                if (filepath != "")
                {
                    ...
                    var uploadResponseInBytes = uploader.UploadFile(info.UploadUrl, filepath); // Загружаем фото на сервер
                    ...
                    var attachs = vkApi.Docs.Save(uploadResponseInString.File, filename, null);
            ...
            {
                ErrorsSystem.Call("Ошибка отправки!", x);
            }
        }
                ...
                switch (user.Status)
                {
                    case VkUserStatus.EnteringShikimori:
                        ...
                        if (result)
                        {
                            ...
                            {
                                if (!user.shikimori.ShikimoriUsers.Contains(user_id))
                                {
                                    user.shikimori.ShikimoriUsers.Add(user_id);
                            ...
                            {
                                SendMessage($"Извини, но такого пользователя не существует. Пропробуй еще раз. Если не хочет, то так и скажи - 'хватит'!", id);
                            }
                        }
                        else
                        {
                            ...
                            {
                                if (user.shikimori.ShikimoriUsers.Count > 0)
                                {
                                    ...
                                    SendMessage($"Теперь давай разберемся, какие темы ты хочешь увидеть в паке. Введи темы одним сообщением в таком формате: '1 5 4 7 9 6' {n}0 - Все темы{n}1 - {Theme_AnimeByJapane.GetRawTitle()}{n}2 - {Theme_CharacterByImage.GetRawTitle()}{n}3 - {Theme_AnimeByScreenshot.GetRawTitle()}{n}4 - {Theme_AnimeByCoub.GetRawTitle()}{n}5 - {Theme_YearOfAnime.GetRawTitle()}{n}6 - {Theme_AnimeByDisappearance.GetRawTitle()}{n}7 - {Theme_CharacterByDescription.GetRawTitle()}" +
                                        $"{n}8 - {Theme_AnimeByDescription.GetRawTitle()}{n}9 - {Theme_AnimeByAnagramm.GetRawTitle()}{n}10 - {Theme_CharacterByJapanese.GetRawTitle()}{n}11 - {Theme_AnimeByPoster.GetRawTitle()}{n}", id);//12 - {Theme_AnimeByOst.GetRawTitle()}{n}13 - {Theme_AnimeByEnding.GetRawTitle()}{n}14 - {Theme_AnimeByOpening.GetRawTitle()}", id);
                                }
                                else
                                {
                        ...
                        for (int i = 0; i < chars.Length; i++)
                        {
                            switch (chars[i])
                            {
                                ...
                                case "11":
                                    ...
                                    break;
                                /*case "12":
                                    user.package.Themes.Add(new Theme_AnimeByOst());
                                    user.AddTheme(6);
                                    break;
                                case "13":
                    ...
                    case VkUserStatus.Nothing:
                        ...
                        {
                            SendMessage($"Хорошо, давай займемся написанием маленького пака. Скажи ссылки на профили Shikimori, с которых мне брать анимешки. Но запомни: 1 сообщение - одна ссылка. Например: https://shikimori.one/ВолчицаХоло. Если же ты передумал, то скажи 'не хочу'.", id);
                        ...
                        {
                            SendMessage($"Хорошо, давай займемся написанием среднего пака. Скажи ссылки на профили Shikimori, с которых мне брать анимешки. Но запомни: 1 сообщение - одна ссылка. Например: https://shikimori.one/ВолчицаХоло. Если же ты передумал, то скажи 'не хочу'.", id);
                        ...
                        {
                            SendMessage($"Хорошо, давай займемся написанием большого пака. Скажи ссылки на профили Shikimori, с которых мне брать анимешки. Но запомни: 1 сообщение - одна ссылка. Например: https://shikimori.one/ВолчицаХоло. Если же ты передумал, то скажи 'не хочу'.", id);
